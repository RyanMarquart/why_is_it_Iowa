...
# KRISTINE - DATA EXPLORATION BELOW
...
# EVAN SECTION BELOW HERE --> PROBABLY NEEDS TO BE INTEGRATED BETTER, BUT THIS IS PROLLY OK FOR NOW



```{r -- log transforming saleprice}
train_log_sp <- real_train %>% mutate(SalePrice = log(SalePrice))
```

```{r -- drop X and set training df}
drop <- ("X")
df1_train = train_log_sp[,!(names(train_log_sp) %in% drop)]
```


```{r - sig vars in basic LM}
...
ridge_mod$bestTune
min(ridge_mod$results$RMSE) # 0.1185774 - when using log(SalePrice)
# 25,127.53 -- when not log(SalePrice)
```
...
```{r ridge regression 2, warning=FALSE}
...
## END OF EVAN'S CODE FOR NOW --> AGAIN PROLLY WILL BE FIXED LATER TONIGHT, BUT WHATEVER

------------------------------------------------------------------------------------------------------------------


# RYAN SECTION --> BELOW HERE
...
x <- data.matrix(train_real[,!names(train_real) %in% c("SalePrice")])
...
test_x <- data.matrix(test_real[,!names(train_real) %in% c("SalePrice")])
...
head(gradient_error)
tail(gradient_error)
Gradient_MSE <- unname(test.error[100])
...
```{r}
#LASSO RMSEs
Train_LASSO_RMSE <- sqrt(mean((train_real$SalePrice - Lasso_estimates)^2))
#Train_LASSO_RMSE
...
#Gradient RMSEs
Train_Gradient_RMSE <- sqrt(mean((train_real$SalePrice - Grad_estimates$prediction_matrix....Grad_Estimates..)^2))
#Train_Gradient_RMSE
